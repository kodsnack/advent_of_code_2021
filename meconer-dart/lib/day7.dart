...
class Day7 extends StatelessWidget {
  ...
  static String routeName = 'day7';
  final bool isExample = false;
  static String dayTitle = 'Day 7: The Treachery of Whales';
      ...
      body: Center(
        child: Column(
          children: [
            ...
            SelectableText('Svar dag 7 del 1: $resultPart1'),
            SelectableText('Svar dag 7 del 2 : $resultPart2'),
          ],
        ),
      ),
    );
  ...
  int doPart1() {
    List<int> crabPosList = getInput(isExample);
    crabPosList.sort();
    int range = crabPosList[crabPosList.length-1] -  crabPosList[0];
    // Loop through all positions
    int minDistance = 99999999;
    for ( int i = 0 ; i < range; i++ ) {
      int alignmentPos = crabPosList[0] + i;
      // Check distance for each crab
      int totalDistance = 0;
      for ( int pos in crabPosList) {
        int distance = pos - alignmentPos;
  ...
  int doPart2() {
    ...
    for ( int i = 0 ; i < range; i++ ) {
      ...
      for ( int pos in crabPosList) {
        int distance = pos - alignmentPos;
        fuelConsumed += getFuelConsumption(distance.abs());
      }
      if (fuelConsumed < minFuelConsumed ) {
        minFuelConsumed = fuelConsumed;
      }
    }
    return minFuelConsumed;
  }

  int getFuelConsumption(int distance) {
    int stepConsumption = 1;
